...
import { CALCULATION_PERIOD_IN_YEARS, GREEN_TARIFF, INCOME_FEE } from '@shared/constants/economic.const';
...
export const economicDataChartAdapter = ({ outputs: { totals: { fixed: {E_y: yearlyGeneration} } } }: SolarData, totalCosts: number): number[] => {
    ...
    for (let i = 0; i < CALCULATION_PERIOD_IN_YEARS; i++) {
        result = Math.ceil(result + ((yearlyGeneration) * (GREEN_TARIFF * (100 - INCOME_FEE))) / 100);
